...
class LossMultiNCE(nn.Module):
    ...
    def forward(self, r_src, r_tgt, pos_matrix, neg_matrix):
        ...
        assert r_src.size(2) == r_src.size(3)
        ...
        # (n_rkhs, n_keys)
        r_tgt = r_tgt.permute(1, 0)

        # reshape pos_matrix and neg_matrix for ease-of-use
        # (n_batch, n_dims * n_dims, n_keys)
        pos_matrix = pos_matrix.unsqueeze(2).expand(-1, -1, n_dims * n_dims).permute(0, 2, 1).float()
        neg_matrix = neg_matrix.unsqueeze(2).expand(-1, -1, n_dims * n_dims).permute(0, 2, 1).float()
        ...
        # (n_batch, n_dims * n_dims, n_keys)
        raw_scores = raw_scores.reshape(n_batch, n_dims * n_dims, n_keys)
        ...
        -- size will be (n_batch * n_dims * n_dims, n_keys)
        '''
