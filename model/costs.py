...
class LossMultiNCE(nn.Module):
    ...
    def forward(self, r_src, r_tgt, pos_matrix, neg_matrix):
        ...
        assert r_tgt.size(2) == r_tgt.size(3)
        ...
        # (n_batch, n_keys, n_dims * n_dims)
        pos_matrix = pos_matrix.unsqueeze(2).expand(-1, -1, n_dims * n_dims).float()
        neg_matrix = neg_matrix.unsqueeze(2).expand(-1, -1, n_dims * n_dims).float()
        ...
        # (n_batch, n_keys, n_dims * n_dims)
        raw_scores = raw_scores.reshape(n_batch, n_keys, n_dims * n_dims)
        ...
        # (n_batch, n_keys * n_dims * n_dims)
        pos_matrix = pos_matrix.reshape(n_batch, -1)

        # (n_batch, n_keys * n_dims * n_dims)
        neg_matrix = neg_matrix.reshape(n_batch, -1)
        ...
        -- size will be (n_batch, n_keys * n_dims * n_dims)
        '''
