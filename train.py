...
class Train:
    ...
    def contrastive_step(self, src_inputs, src_indices, tgt_inputs, tgt_indices):
        ...
        batch_features = torch.cat([src_end_points['local_features'], tgt_end_points['local_features']], dim=0)

        with torch.no_grad():
            self.model_ema.set_bn_domain(domain=0)
            src_end_points_ema = self.model_ema(src_inputs)
            self.src_memory.store_keys(src_end_points_ema['global_features'], src_indices)

            self.model_ema.set_bn_domain(domain=1)
            tgt_end_points_ema = self.model_ema(tgt_inputs)
            self.tgt_memory.store_keys(tgt_end_points_ema['global_features'], tgt_indices)
